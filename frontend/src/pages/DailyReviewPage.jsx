...
const STATUS_CLS = {
  submitted: 'text-amber-400', approved: 'text-emerald-400',
  rejected: 'text-red-400', needs_correction: 'text-orange-400',
...
function NoteModal({ title, onConfirm, onClose }) {
...
export default function DailyReviewPage() {
  ...
  return (
    <div>
      ...
      {/* Filter bar */}
      <div className="sticky top-0 z-30 bg-slate-950/97 backdrop-blur-md border-b border-slate-800 -mx-6 px-6 py-2.5 mb-4 shadow-lg shadow-black/20">
          ...
          <div className="ml-auto flex items-center gap-2">
            {signoff?.status === 'signed_off' ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-emerald-400 font-semibold">Signed Off</span>
                <button onClick={() => setNoteModal({ action: 'reopen' })}
                  className="text-[10px] text-slate-500 hover:text-slate-300 underline">Reopen</button>
              </div>
            ) : (
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Scheduled', val: data.assignments.length, color: 'text-brand-400' },
              { label: 'Time Entries', val: data.time_entries.length, color: 'text-slate-100' },
              { label: 'Active Clocks', val: data.active_clock_sessions.length, color: 'text-emerald-400' },
              { label: 'Pending Corrections', val: data.pending_corrections.length, color: 'text-amber-400' },
            ].map(s => (
          ...
          {(missingIds.size > 0 || unscheduledIds.size > 0) && (
            <div className="card">
              <button onClick={() => setShowVariance(v => !v)}
                ...
                {showVariance ? <ChevronUp className="w-3.5 h-3.5 text-slate-600 ml-auto" />
                  ...
                  {[...missingIds].map(eid => {
                    ...
                    return (
                      <div key={eid} className="flex items-center justify-between bg-red-500/8 rounded-lg px-3 py-2 border border-red-500/15">
                        <div>
                          <span className="text-xs font-semibold text-red-400">Missing Timecard</span>
                          <p className="text-[10px] text-slate-500">{a?.employee_name || `Employee #${eid}`} — scheduled {a?.planned_hours}h on {a?.job_number}</p>
                        </div>
                        {a && <button onClick={() => action(() => dailyReviewApi.markAbsent({ employee_id: eid, assignment_id: a.id }), 'Marked absent')}
                          className="text-[10px] text-slate-500 hover:text-red-400 ml-4">Mark Absent</button>}
                      </div>
                    )
                  })}
                  {[...unscheduledIds].map(eid => {
                    ...
                    return (
                      <div key={eid} className="flex items-center justify-between bg-amber-500/8 rounded-lg px-3 py-2 border border-amber-500/15">
                        <span className="text-xs font-semibold text-amber-400">Unscheduled Work</span>
                        <p className="text-[10px] text-slate-500">{e?.employee_name || `Employee #${eid}`}</p>
                      </div>
                    )
                  })}
                </div>
          ...
          {data.active_clock_sessions.length > 0 && (
            <div className="card">
              <h3 className="font-semibold text-sm text-emerald-300 flex items-center gap-2 mb-3">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                Active Clock Sessions ({data.active_clock_sessions.length})
              </h3>
              <div className="space-y-2">
                {data.active_clock_sessions.map(s => (
                  <div key={s.id} className="flex items-center justify-between text-sm bg-slate-800/40 rounded-lg px-3 py-2">
                    <span className="text-slate-300">{s.employee_name}</span>
                    <div className="text-right">
                      <span className="font-mono text-[10px] text-brand-400">{s.job_number}</span>
                      <p className="text-[10px] text-slate-600">{s.clock_in_time ? format(new Date(s.clock_in_time), 'h:mm a') : ''}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="card">
            <h3 className="font-semibold text-sm text-slate-200 flex items-center gap-2 mb-3">
              <Clock className="w-4 h-4 text-brand-400" /> Time Entries ({filteredEntries.length})
            </h3>
            {filteredEntries.length === 0 ? (
              <p className="text-sm text-slate-600 italic">No time entries for today.</p>
            ) : (
              <div className="space-y-2">
                {filteredEntries.map(e => (
                  <div key={e.id} className="flex items-center justify-between gap-3 bg-slate-800/40 rounded-lg px-3 py-2.5">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm text-slate-200 font-medium truncate">{e.employee_name}</p>
                        <span className={`text-[10px] font-semibold capitalize ${STATUS_CLS[e.status] || 'text-slate-400'}`}>{e.status}</span>
                      </div>
                      <p className="text-[10px] text-slate-500">{e.job_number} · {e.start_time}–{e.end_time}</p>
                    </div>
                    ...
                    {e.status === 'submitted' && (
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button onClick={() => setNoteModal({ action: 'reject', entryId: e.id })}
                          className="p-1 text-slate-600 hover:text-red-400" title="Reject">
                          <Ban className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => setNoteModal({ action: 'correction', entryId: e.id })}
                          className="p-1 text-slate-600 hover:text-amber-400" title="Needs Correction">
            ...
            <div className="card">
              ...
              <div className="space-y-2">
                {data.pending_corrections.map(cr => (
                  <div key={cr.id} className="bg-slate-800/40 rounded-lg px-3 py-2.5 flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-slate-200">{cr.employee_name || `Entry #${cr.time_entry_id}`}</p>
