...
function LogRow({ log }) {
  ...
  return (
    <div className="border-b border-gray-200 dark:border-slate-800/60 last:border-0">
      <button
        className="w-full flex items-center gap-3 py-3 px-4 hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors text-left"
      ...
      >
        ...
        <span className="text-sm text-gray-700 dark:text-slate-300 flex-1">
          <span className="font-medium text-gray-800 dark:text-slate-200">{log.user_name || 'System'}</span>
          {' '}
          <span className="text-gray-600 dark:text-slate-400 capitalize">{log.resource_type?.replace('_', ' ')}</span>
          {log.resource_id ? <span className="text-gray-400 dark:text-slate-600"> #{log.resource_id}</span> : null}
          {log.note && <span className="text-gray-500 dark:text-slate-500 ml-2 italic truncate">— {log.note}</span>}
        </span>
        ...
        {(oldVal || newVal) ? (
          open ? <ChevronDown className="w-3 h-3 text-gray-400 dark:text-slate-600 shrink-0" /> : <ChevronRight className="w-3 h-3 text-gray-400 dark:text-slate-600 shrink-0" />
        ) : <span className="w-3" />}
      </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            {oldVal && (
              <div className="bg-red-500/5 border border-red-500/15 rounded-lg p-3">
                ...
                <pre className="text-xs text-gray-600 dark:text-slate-400 whitespace-pre-wrap font-mono">{JSON.stringify(oldVal, null, 2)}</pre>
              </div>
            )}
            {newVal && (
              <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-lg p-3">
                <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-2">After</p>
                <pre className="text-xs text-gray-600 dark:text-slate-400 whitespace-pre-wrap font-mono">{JSON.stringify(newVal, null, 2)}</pre>
              </div>
            )}
          </div>
          {log.details && <p className="text-xs text-gray-500 dark:text-slate-500 mt-2">{log.details}</p>}
        </div>
      )}
    </div>
  )
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        <button
          onClick={() => setFilter('')}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${!filter ? 'bg-brand-600 text-white' : 'bg-gray-200 dark:bg-slate-800 text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-200'}`}
        >
          All ({logs.length})
        </button>
        {actions.map(a => (
          <button
            key={a}
            onClick={() => setFilter(a)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${filter === a ? 'bg-brand-600 text-white' : 'bg-gray-200 dark:bg-slate-800 text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-200'}`}
          >
